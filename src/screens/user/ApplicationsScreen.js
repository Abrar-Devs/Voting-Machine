...
const ApplicationsScreen = () => {
  ...
  return (
    <ScrollView>
      <Text style={[globalStyles.label, globalStyles.boldText]}>Applications to Approve</Text>
      <View style={[globalStyles.container, globalStyles.flexGap(10)]}>
        {applications.map(item => (
          <Application key={item.id} {...item} approveApplication={approveApplication} />
        ))}
      </View>
    </ScrollView>
  )
...
const Application = ({ id, partyName, partySymbol, user, approveApplication }) => (
  <View style={[globalStyles.cardView, globalStyles.boxShadow()]}>
    ...
    <TouchableOpacity style={globalStyles.btn} onPress={() => approveApplication(id)}>
